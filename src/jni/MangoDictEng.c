...
static void LookupProgressCB(double progress)
{
    ...
    clazz = (*g_env)->FindClass(g_env, "com/mango/MangoDict/MangoDictEng");

    (*g_env)->CallStaticVoidMethod(g_env, clazz, (*g_env)->GetStaticMethodID(g_env, clazz, "lookupProgressCB","(I)V"), i);
}
 ...
 */
jobjectArray
LookupWords(JNIEnv* env, jobject thiz, jstring word, LIST_WORDS_TYPE listType)
{
	jclass objClass;
	...
	{
		wordcount = DictEng_LookupWithFuzzy(g_pDictEng, wordUTF, &pWordsList, (PFN_PROGRESS)LookupProgressCB);
	}
	else
	{
		wordcount = DictEng_LookupWithRule(g_pDictEng, wordUTF, &pWordsList, (PFN_PROGRESS)LookupProgressCB);
	}
...
jboolean
Java_com_mango_MangoDict_MangoDictEng_LoadDicts(JNIEnv* env, jobject thiz, jobjectArray paths, jobjectArray names, jintArray types)
{
	int i = 0, count = 0;
	char * strUTF = NULL;
	char ** pPaths = NULL;
    ...
    for (i = 0; i < count; i++) {
		strUTF = (char*)(*env)->GetStringUTFChars(env, (jstring)(*env)->GetObjectArrayElement(env, paths, i), NULL);
		pPaths[i] = strdup(strUTF);
		(*env)->ReleaseStringUTFChars(env, (jstring)(*env)->GetObjectArrayElement(env, paths, i), strUTF);
		strUTF = (char*)(*env)->GetStringUTFChars(env, (jstring)(*env)->GetObjectArrayElement(env, names, i), NULL);
		pNames[i] = strdup(strUTF);
		(*env)->ReleaseStringUTFChars(env, (jstring)(*env)->GetObjectArrayElement(env, names, i), strUTF);
	}
...
jboolean
Java_com_mango_MangoDict_MangoDictEng_UnloadDicts(JNIEnv* env, jobject thiz)
{
	if(NULL != g_pDictEng)
	{
...
void
Java_com_mango_MangoDict_MangoDictEng_CancelLookup(JNIEnv* env, jobject thiz)
{
	DictEng_SetLookupCancel(g_pDictEng, TRUE);
}

jobjectArray
Java_com_mango_MangoDict_MangoDictEng_PatternListWords(JNIEnv* env, jobject thiz, jstring word)
{
	return LookupWords(env, thiz, word, LIST_WORDS_PATTERN);
}

jobjectArray
Java_com_mango_MangoDict_MangoDictEng_FuzzyListWords(JNIEnv* env, jobject thiz, jstring word)
{
	return LookupWords(env, thiz, word, LIST_WORDS_FUZZY);
}

jobjectArray
Java_com_mango_MangoDict_MangoDictEng_ListWords(JNIEnv* env, jobject thiz, jstring word)
{
	return LookupWords(env, thiz, word, LIST_WORDS_NORMAL);
}

jobjectArray
Java_com_mango_MangoDict_MangoDictEng_FullTextListWords(JNIEnv* env, jobject thiz, jstring word)
{
	jclass objClass;
	jobjectArray objArray = NULL;
	jstring jStrData;
	...
	DictEng_SetLookupCancel(g_pDictEng, FALSE);
	bFind = DictEng_LookupData(g_pDictEng, wordUTF, &searchInfo, (PFN_PROGRESS)LookupProgressCB);
	...
	for (i = 0; i < searchInfo.dictCount; i++)
	{
		...
		dataSize = strlen(searchInfo.searchData[i].bookName) + strlen(HTML_BOOKNAME_B) + strlen(HTML_BOOKNAME_E);
		dataSize += 1;
		...
		{
			jStrData = (*env)->NewStringUTF(env , searchInfo.searchData[i].pKeyWord[k]);	// Add dict book name to the array.
			(*env)->SetObjectArrayElement(env, objArray, j++, jStrData);
		}
	}
...
jobjectArray
Java_com_mango_MangoDict_MangoDictEng_Lookup(JNIEnv* env, jobject thiz, jstring word, jint type)
{
	jclass objClass;
	jobjectArray objArray = NULL;
	jstring jStrData;
...
jstring
Java_com_mango_MangoDict_MangoDictEng_GetBookName(JNIEnv* env, jobject thiz, jstring ifoPath)
{
...
{
	Java_com_mango_MangoDict_MangoDictEng_UnloadDicts(NULL, NULL);
}
